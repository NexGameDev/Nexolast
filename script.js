...
function preview(index){
  ...
  selectedShape.forEach(([x,y])=>{
    ...
    if(r>=SIZE || c>=SIZE) valid = false;
    else if(board[r*SIZE+c]) valid = false;
  });
...
function anyBlockFits(){
  return [...blockListEl.children]
    .some(b=>canFitAnywhere(b.shape));
}
...
boardEl.addEventListener("click",clearPreview);
